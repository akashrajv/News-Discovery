...
logger = get_logger("news_engine.gnews")

class GNewsConnector(BaseAPIConnector):
    ...
    def build_query(self, entity: str, keywords: List[str], location: Optional[str] = None, category: Optional[str] = None) -> str:
        # Take primary target entity or clean commas into space
    ...
    async def fetch(self, query: str, time_window_minutes: int = 60) -> List[Dict[str, Any]]:
        ...
        if not avail:
            logger.info(f"GNews unavailable: {msg}")
            return []

        endpoint = self.api_endpoint or "https://gnews.io/api/v4/search"
        params = {
            "q": query,
            "apikey": self.api_key,
            "lang": "en",
    ...
    def normalize(self, raw_items: List[Dict[str, Any]]) -> List[ArticleCreate]:
        ...
        for item in raw_items:
            ...
            article = ArticleCreate(
                ...
                url=url or f"https://gnews.io/article/{c_hash}",
